...
export default function PopupSearch({ products, categories }: PopupSearchProps) {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} className="flex items-center relative w-full max-w-3xl">
          ...
          >
            <div className="flex flex-col h-full">
              <div className="flex items-center lg:p-4 p-2 border-b">
                ...
                <form className="flex-1 lg:ml-4 relative">
  ...
  />
              ...
              <ScrollArea className="flex-1 lg:px-[6rem] px-2 lg:py-8 py-4">
                ...
                {filteredProducts.length > 0 ? (
                  <div>
                    <h3 className="text-lg font-semibold mb-4">Searches from {searchTerm}</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {filteredProducts.map((product) => (
                        <ProductCard key={product.id} product={product} onClick={() => setIsOpen(false)} />
                      ))}
                    </div>
                  </div>
                ) : (
                  searchTerm.length > 1 && (
                    <div className="text-center py-8">
                      <p className="text-gray-500">No products or categories found for {searchTerm}</p>
                    </div>
                  )
                )}
              </ScrollArea>
