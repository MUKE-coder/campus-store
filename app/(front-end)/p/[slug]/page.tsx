...
import BookNowBtn from "@/components/front-end/BookNowBtn";
...
export default async function page({ params: { slug } }: { params: { slug: string } }) {
  const session = await getServerSession(authOptions);
    ...
    const [product, products] = await Promise.all([
        getData(`/products/product/${slug}`),
        getData(`products/all`)as Promise<ProductTypes[]>,
      ]);
    ...
    <>
      ...
      <div className="lg:hidden md:hidden flex gap-3 items-center fixed bottom-1 w-[100%] bg-gray-200 px-3 py-2">
        ...
        <div className="w-[75%]">
          ...
          <BookNowBtn product={product} backgroundColor="#f68b1e" session={session}/>
        </div>
