...
async function fetchBannerData() {
  try {
    ...
    return { bannerData: bannerData || [], allCategoriesData: allCategoriesData || [] };
  } catch (error) {
...
async function AllCategoriesSection({ bgSecondaryColor }: { bgSecondaryColor: string }) {
  try {
    const { allCategoriesData } = await fetchBannerData();
    const categoriesWithProducts: Category[] = allCategoriesData.filter(
      (category) => category?.products && category.products.length > 0
    );
    return <AllCategories categories={categoriesWithProducts} bgSecondaryColor={bgSecondaryColor} />;
  } catch (error) {
    console.error("Error in AllCategoriesSection:", error);
    return null;
  }
}
...
async function CategorySection() {
  try {
    ...
    console.error("Error in CategorySection:", error);
    return null;
  }
}
...
export default async function Home() {
  try {
    ...
    const bgColor = styleData?.primaryColor || "#ffffff";
    ...
    return (
      <div style={{ backgroundColor: bgColor }} className="min-h-[100vh] pb-5 [family-name:var(--font-geist-sans)]">
        <div
          className="w-full home-bg lg:px-[6rem] md:pt-[3rem] pt-[2rem] lg:pt-[3.2rem]"
